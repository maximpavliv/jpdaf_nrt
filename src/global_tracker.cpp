...
//  cout << "----------\n";
  ...
  else if(!init_ && !startTracking_) // second go, third, etc until manageNewTracks haven't created some localTrackers. Until that, prev_detection = not_associated_
  {
  ...
  else
  {
    ...
    delete_tracks(); // TODO check what this does exactly
...
//  cout << "prevDetSize: " << prevDetSize << ", deteSize: " << deteSize << "\n";
  if(prevDetSize == 0)
  {
    prev_detections_ = not_associated_;
  }
  ...
  {    
    ...
    distMatrix_t costs(deteSize * prevDetSize); // std::vector<track_t>, track_t = float
    ...
    assignments_t assignments; //std::vector<int>
